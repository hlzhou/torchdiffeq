...
class VariableCoefficientAdamsBashforth(AdaptiveStepsizeODESolver):
    ...
    def __init__(self, func, y0, rtol, atol,
                 implicit=True, max_order=_MAX_ORDER,
                 safety=0.9, ifactor=10.0, dfactor=0.2, **unused_kwargs):
    ...
    def before_integrate(self, t):
        ...
        assert first_step > 0, "initial step size must be positive t0 = {}, first_step = {}".format(t0, first_step)  # [NEW]
    ...
    def _adaptive_adams_step(self, vcabm_state, final_t):
        ...
        if not accept_step:
            ...
            assert dt_next > 0, "zero step size after rejection"  ## [NEW]
        ...
        if self.func.jump_type == "read":
            ...
            vcabm_state = vcabm_state._replace(y_n=y0,
                                               next_t=self.func.next_read_jump(vcabm_state.prev_t[0],
                                                                               vcabm_state.next_t),
                                               order=order0)  # perform the jump & change step size
